...
fs.mkdir(`./src`, function(err) {
  if (err) {
    if (err.errno === -17){console.log("./src already exist")}
...
function getImages (html){
  ...
  result.length = 10
  if(text1 === undefined){
    result.forEach((item, index)=>{downloadImage(item.attribs.src, index, true)})
  }else{ if(text1 === "clean"){
     compoundText = undefined;
     result.forEach((item, index)=>{downloadImage(item.attribs.src.split("/")[1], index)})
    }
    else{
      result.forEach((item, index)=>{downloadImage(item.attribs.src.split("/")[1], index)})
    }
...
function downloadImage (url, index, clean = false) {
  ...
  : {
    url: `https://api.memegen.link/images/${url}${compoundText ? "/"+compoundText+"/"+index:""}.jpg?preview=true&watermark=none`,
    dest: `./src/images${dateNow}`               // will be saved to /path/to/dest/image.jpg
  } 
